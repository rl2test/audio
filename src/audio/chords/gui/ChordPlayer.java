...
public class ChordPlayer extends Thread { 
	 ...
	 * @param filePanel
	 */
	public ChordPlayer(String chord, AudioController ac) {
		this.chord = chord;
		this.ac = ac;
	}
	...
	public void run() {
		try {
			...
			int time		= (timePanel.set) ? timePanel.time : 4;
			int type		= (timePanel.set) ? timePanel.type : 1;
			int beginTempo	= (timePanel.set) ? timePanel.beginTempo : 90;
			int endTempo 	= (timePanel.set) ? timePanel.endTempo : 180;
			int increment 	= (timePanel.set) ? timePanel.increment : 1;
			int numBars 	= (timePanel.set) ? timePanel.numBars : 8;
