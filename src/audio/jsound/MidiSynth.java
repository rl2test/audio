...
public class MidiSynth extends JPanel implements ControlContext {
    ...
    public void open() {
      ...
      Soundbank sb = synthesizer.getDefaultSoundbank();
			...
			if (sb != null) {
			System.out.println("Got Soundbank"); //RL
            instruments = synthesizer.getDefaultSoundbank().getInstruments();
			for(int i = 0;i<128; i++){ //RL
				//System.out.println("instruments.length: " + instruments.length); //RL
				System.out.print(instruments[i].getName() + " "); //RL
			} //RL
	        synthesizer.loadInstrument(instruments[0]);
        }else{System.out.println("didn't get Soundbank");}

        	/*
			Soundbank sb = null;
			File file = new File("c:\\jdk1.3.1_01\\jre\\lib\\audio\\soundbank.gm"); //_deluxe
	    try {
